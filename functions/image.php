...
function uploadImage($index, $destination, $maxsize=false, $extensions=false) {
    ...
    if (!isset($_FILES[$index]) || $_FILES[$index]['error'] > 0 || $_FILES[$index] == null) {
        switch ($_FILES[$index]['error']) {
            case 1:
                $msgs[] = "La taille du fichier téléchargé excède la valeur max du serveur";
                break;
            case 2:
                $msgs[] = "La taille du fichier téléchargé excède la valeur max spécifiée dans le formulaire HTML";
                break;
            case 3:
                $msgs[] = "Le fichier n'a été que partiellement téléchargé";
                break;
            case 4:
                $msgs[] = "Aucun fichier n'a été téléchargé";
                break;
            case 6:
                $msgs[] = "Un dossier temporaire est manquant";
                break;
            case 7:
                $msgs[] = "Échec de l'écriture du fichier sur le disque";
                break;
            case 8:
                $msgs[] = "Une extension PHP a arrêté l'envoi de fichier";
                break;
            default:
                $msgs[] = "Erreur inconnue";
                break;
        }
    } elseif ($maxsize !== false && $_FILES[$index]['size'] > $maxsize) {
        $msgs[] = "Le fichier est trop volumineux";
    } elseif ($extensions !== false && !in_array($ext,$extensions)) {
        $msgs[] = "Le fichier n'est pas au bon format";
    }
...
function infoImage($image) {
    ...
    $ext_valide = array('.jpg','.jpeg','.png','.bmp','.gif');
    
    $image_v = in_array($ext_image_m,$ext_valide);
    
    $info_fichier = array($nom_image, $ext_image, $image, $image_v);
